...
* next product wave `WF1 Scoped Daily Operations Runtime` เริ่มถูกส่งลงระบบแล้ว: persistence invariant เปลี่ยนเป็น `one active template per scope`, staff runtime รองรับ scope-aware entry, และ checklist-to-incident handoff รักษา scope context ได้แล้ว โดยยังคง A-lite scope และปฏิเสธ workflow enterprise เกินจำเป็น  
...
* `WF1 Scoped Daily Operations Runtime` เริ่มถูกส่งลงระบบแล้วในก้อน `WF1-A + minimum WF1-B`: staff จะ auto-enter ได้เมื่อมี live scope เดียว, จะเห็น runtime board เมื่อมีหลาย live scopes, และจะได้รับ `scope missing` state เมื่อเลือก lane ที่ยังไม่มี active template แทน configuration error แบบเก่า  
