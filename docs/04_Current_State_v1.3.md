...
* `WF3 User Administration Lite` เริ่มลง product surface แล้วในก้อน `WF3-B`: admin-only route family `/users`, `/users/create`, และ `/users/{user}/edit` ถูกเพิ่มเข้ามาใน main application shell, navigation เริ่มมี lane สำหรับ user administration จริง, roster surface แสดง role และ active/inactive state อย่างตรงไปตรงมา, และ create/edit surface เรียกใช้ lifecycle owners จาก WF3-A โดยตรงแทนการฝัง validation/authorization logic ซ้ำไว้ใน Livewire หรือ Blade  
* `WF3 User Administration Lite` เดินต่อแล้วในก้อน `WF3-C`: admin lifecycle มี guard rails ใน application layer แล้วสำหรับ self-deactivation, self-demotion, และการปล่อยให้ระบบไม่มี active administrator เหลืออยู่, ขณะที่ user administration surface สะท้อนข้อจำกัดเหล่านี้อย่างตรงไปตรงมาแทนการให้ affordance หลอกที่ save ไม่ได้  
